...
import config
from data.div_loader import load_pretrained_embedding, load_corresponding_query
...
class MLPIV(nn.Module):
    def __init__(self, input_dim, b, output_dim):
        super(MLPIV, self).__init__()
        if (b == "coat"):
...
def train_model(model, X_train, y_train, epochs=30, lr=1e-3):
...
class PyTorchDeepIV:
    def __init__(self, n_components, m, h, n_samples, use_upper_bound_loss, n_gradient_samples, optimizer, first_stage_options):
        ...
        self.first_stage_options = first_stage_options
    ...
    def fit(self, T, Z):
        ...
        train_model(self.m, Z_tensor, T_tensor, **self.first_stage_options)
...
class iVAE(nn.Module):
    ...
    def iv(self, photo, query, query_pinv):
        ...
        q_t = torch.matmul(t_1, beta.unsqueeze(dim=-1))
    ...
    def forward(self, x, w, treatment_pred):


        treatment_pred = treatment_pred.to(torch.float).to(self.device)



        i_embedding = treatment_pred


        fc_layer = nn.Linear(i_embedding.shape[1], x.shape[1]).to('cuda')
        i_embedding = fc_layer(i_embedding)

        x = x.to('cuda')
        i_embedding = i_embedding.to('cuda')
