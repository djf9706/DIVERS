...
class MFwithFeature(nn.Module):
    ...
    def forward(self, u_id, i_id, features):
        ...
        # print(" self.feature_embs_u)", self.feature_embs_u)
        #
        # print("features.shape[1]", features.shape[1])
        # print("min(features.shape[1], len(self.feature_embs_u))",min(features.shape[1], len(self.feature_embs_u)))
        # print(" len(self.feature_embs_u)",len(self.feature_embs_u))
        #
        # print("(features[:, i])", features[:, 0])
        # print("(features[:, i])", features[:, 0].shape)
        # print("elf.feature_embs_u[i](features[:, i])", self.feature_embs_u[0](features[:, 0]))
        # print("elf.feature_embs_u[i](features[:, i])", self.feature_embs_u[0](features[:, 0]).shape)
        for i in range(features.shape[1]):
        # for i in range(min(features.shape[1], len(self.feature_embs_u))):
        #     print("i", i)
            # print("feature_embs_u", self.feature_embs_u)
            # print("self.feature_embs_u[i](features[:, i]", self.feature_embs_u[i](features[:, i]))
            # print("self.feature_embs_i[i](i_id)", self.feature_embs_i[i](i_id))
            # print("self.feature_embs_u[i](features[:, i]", self.feature_embs_u[i](features[:, i]).shape)
            # print("self.feature_embs_i[i](i_id)", self.feature_embs_i[i](i_id).shape)
